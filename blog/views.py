...
def index(request):
    most_popular_posts = Post.objects.annotate(Count('likes')).prefetch_related("author").order_by(
        '-likes__count')[:5]
    ...
    fresh_posts = Post.objects.annotate(comments_count=Count('comments')).prefetch_related("author").order_by('published_at')
